...
async def list_notifications(request: Request, user=Depends(require_auth)):
        ...
        with get_db() as conn:
            ...
            for r in rows:
                ...
                for i, c in enumerate(cols):
                    item[c] = r[i] if i < len(r) and r[i] is not None else ""
                items.append(item)
...
async def test_sms(request: Request, user=Depends(require_auth)):
    ...
    try:
        ...
        if not all([sid, token_val, from_num]):
            return {"success": False, "error": f"Twilio not configured. SID={bool(sid)}, TOKEN={bool(token_val)}, FROM={bool(from_num)}"}

        try:
            from twilio.rest import Client
        except ImportError:
            return {"success": False, "error": "twilio package not installed on server"}
        ...
        try:
            ...
            return {"success": False, "error": f"Twilio API error: {str(twilio_err)}"}
        ...
        return {"success": False, "error": f"test_sms error: {str(e)}"}
...
# (duplicate scoring routes removed — early routes handle these above)
